...
def extract_video_audio(
    video_path, save_audio_dir="", sr=16000, fps=60, size=(512, 512)
):
...
def crop_video(vd, crop_size=(512, 512), fps=60):
    ...
    crop_obj = Cropper(crop_size[0], scale=[3, 2], trans_scale=0.0)
    ...
    while cap.isOpened():
        ...
        trans_points = crop_obj.transform_points(
            points, tform, points_scale=None, normalize=True
        )
        frame = (
            cropped_image.detach().cpu().squeeze().numpy().transpose(1, 2, 0)
        )  # tensor to cv2 array , permute(2, 0, 1)
...
def lucas_kanade_method_imgs(resources_path, kp=None, save_kp_dir="./"):
    ...
    if kp is None:
        raise ValueError(
            f"{kp} should not be None, using labelme to get the keypoints what u need on first frame"
        )
    ...
    imgs = sorted(
        [
            img
            for img in os.listdir(resources_path)
            if img.endswith(".{}".format("png"))
        ],
        key=s,
    )
...
def main(args):
    ...
    testdata = datasets.TestData(
        args.inputpath, iscrop=args.iscrop, crop_size=224, face_detector=args.detector
    )
    ...
    for i in tqdm(range(len(testdata))):
        ...
        with torch.no_grad():
            ...
            if cam_flag:
                cam = (
                    codedict["cam"].detach().cpu().numpy()
                )  # [scale, tx, ty], in [-1, 1] ? or [0,1] ?
                save_cam = {}
                save_cam["scale"], save_cam["xc"], save_cam["yc"] = (
                    cam[0, 0],
                    cam[0, 1],
                    cam[0, 2],
                )
            ...
            landmarks2d = transform_points(
                landmarks2d, tform, points_scale=[224, 224], out_scale=[h, w]
            )

            landmarks3d = opdict["landmarks3d"]
            # landmarks3d = util.batch_orth_proj(landmarks3d, codedict['cam'])
            # landmarks3d[:,:,1:] = -landmarks3d[:,:,1:]  # y, z -1 ?
            landmarks3d = util.batch_orth_proj_(
                landmarks3d, codedict["cam"], inverse=False
            )

            pts_3d_224.append(landmarks3d)

            landmarks3d = transform_points(
                landmarks3d, tform, points_scale=[224, 224], out_scale=[h, w]
            )
    ...
    np.savez(
        os.path.join(savefolder, "3d_fit_data.npz"),
        pts_3d=pts_3d,
    ...
    np.savez(
        os.path.join(savefolder, "3d_fit_data_224.npz"),
        pts_3d=pts_3d_224,
        rot_angles=rot_angles,
        trans=trans,
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="DECA: Detailed Expression Capture and Animation for generat lsp data"
    )
    ...
    # # get shoulder points, this function work not good, it surpose your video is good(....)
