...
def batch_orth_proj_(X, camera, inverse=False):
    M = torch.eye((4), device=X.device, dtype=X.dtype)
    X = torch.cat([X, torch.ones(1, device=X.device).reshape(1, 1, 1).repeat(1, 68, 1)], 2)
    s, tx, ty = camera[0,0], camera[0,1], camera[0, 2]
    M[0, 0] = s
    M[0, 3] = s*tx
    M[1, 1] = -s
    M[1, 3] = -s*ty
    ...
    if inverse:
        ...
        M[1, 3] = s*ty
        M[2, 2] = -s 
    Xn = torch.matmul(M, X.permute(0,2,1)).permute(0,2,1)
    return Xn[..., :3]
...
# ---------------------------------- visualization
end_list = np.array([17, 22, 27, 42, 48, 31, 36, 68], dtype = np.int32) - 1  # 列表 TODO
def plot_kpts(image, kpts, color = 'r'):
    ...
    for i in range(kpts.shape[0]):
        ...
        if i in end_list:
            continue
        ed = kpts[i + 1, :2]
        image = cv2.line(image, (int(st[0]), int(st[1])), (int(ed[0]), int(ed[1])), (255, 255, 255), radius)  # 关键点顺序不同
