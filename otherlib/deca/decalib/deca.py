...
class DECA(nn.Module):
    ...
    def _create_model(self, model_cfg):
        ...
        self.n_detail = model_cfg.n_detail  # 128
        self.n_cond = model_cfg.n_exp + 3 # exp + jaw pose  50 + 3
        self.num_list = [model_cfg.n_shape, model_cfg.n_tex, model_cfg.n_exp, model_cfg.n_pose, model_cfg.n_cam, model_cfg.n_light]
        self.param_dict = {i:model_cfg.get('n_' + i) for i in model_cfg.param_list}  # 236 = 100+50+50+3+6+27
    ...
    def encode(self, images, use_detail=True):
        if use_detail:
            ...
            parameters = self.E_flame(images)
        # print(images.shape, 'encode', parameters.shape)
    ...
    def decode_simple(self, codedict):
        ## decode
    ...
    def decode(self, codedict, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=True,
        ...
        # print(images.shape, 'from decode')
        ...
        ## projection
        # print(codedict['cam'], landmarks2d.shape) # ([1, 68, 3]
        ...
        if vis_lmk:
            ...
            landmarks3d = torch.cat([landmarks3d, landmarks3d_vis], dim=2)  # [1, 68, 3]->[1, 68, 4]
            opdict['landmarks3d'] = landmarks3d

        if return_vis:
            if render_orig and original_image is not None and tform is not None:
                points_scale = [self.image_size, self.image_size]  # 224, 224
                _, _, h, w = original_image.shape
                # print(h, w, 'orinal image size!')  # 720 1280
            ...
            # visdict = {
            #     'inputs': images, 
            #     'landmarks2d':  util.tensor_vis_landmarks(images, landmarks2d)[-1],
            #     'landmarks3d':  landmarks3d,
            #     'shape_images': shape_images,
            #     'shape_detail_images': shape_detail_images
            # }
